...
import type { EventType, EventChoicesType } from "@/types/event.type";
...
export default function EventCard({
  ...
  const isSelected = eventChoicesConfig.some(
    (ev) => ev.event_name === event.event_name
  );
      ...
      <CardHeader className="pb-2 flex items-center justify-between">
        ...
        {deleteEventList && isSelected && (
          <Button
            variant="ghost"
            size="icon"
            onClick={() => deleteEventList(event.event_name)}
              ...
              className={`${
                eventChoicesConfig.some(
                  (ev) =>
                    ...
                    ev.chosen === parseInt(choice.choice_number)
                )
